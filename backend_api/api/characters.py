...
async def add_attack_to_character(
    ...
    result = await db.execute(
        select(Character).where(
            Character.id == character_id, Character.owner_id == current_user.id
        )
    )
    ...
    if not character:
        raise HTTPException(
            status_code=404, detail="Персонаж не найден или у вас нет к нему доступа"
        )
...
async def roll_character_attack(
    ...
    if not attack:
        raise HTTPException(
            status_code=404, detail="Атака не найдена или у вас нет к ней доступа"
        )
...
async def roll_character_spell(
    ...
    if not spell:
        raise HTTPException(
            status_code=404, detail="Заклинание не найдено или у вас нет к нему доступа"
        )
...
@router.delete(
    "/{character_id}/attacks/{attack_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def delete_attack(
    ...
    char_stmt = select(Character.id).where(
        Character.id == character_id, Character.owner_id == current_user.id
    )
    ...
    stmt = delete(Attack).where(
        Attack.id == attack_id, Attack.character_id == character_id
    )
...
@router.delete(
    "/{character_id}/spells/{spell_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
@router.delete(
    "/{character_id}/features/{feature_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def delete_feature(
    ...
    feat_result = await db.execute(
        select(Feature).where(
            Feature.id == feature_id, Feature.character_id == character_id
        )
    )
